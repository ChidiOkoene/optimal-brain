...
`/Users/chidi/Obsidian/AI Research` (example; replace with your actual root)
...
```bash
# By name
find "/Users/chidi/Obsidian/AI Research" -name "*Index*.md" -o -name "*Learning*.md"

# Content search for links
grep -rl "\[\[Some Concept\]\]" "/Users/chidi/Obsidian/AI Research"
```
...
- **Agent loops** (`agent-loop`, `/until-done`): When knowledge gaps appear, reach for `research-from-vault`. Goals like "research X using vault until synthesis note + learning record exists" are valid.
